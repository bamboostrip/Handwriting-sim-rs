...
- **导入 docx**：自动解析段落对齐方式与首行缩进（沿样式链继承）；若文档带有高亮，导入时可选「全文手写模式」（忽略全部高亮整篇手写）或「打印/手写混排」（高亮文字手写、未高亮文字保留打印体）
- **划选标记与笔迹角色**：正文编辑器中划选文字后一键设为打印体 / 手写角色 / 清除标记（恢复默认手写）；Word 中首次出现的高亮自动映射为手写角色 1，次出现映射为手写角色 2
...
- **写错字模拟**：错字率 0~30%，随机错字划掉后在右上方（正上方略偏右）小一号重写（或后文正常位置重写），
  涂改样式可选单线 / 双线 / 斜线 / 叉号
...
### 5. 常见特殊需求：保留背景色或懒得改模板？
- **PDF 想要保留背景色（不擦除底色、不提取选框）**：点击「导入文档底图」选择文件后，在弹出的提示中选择**「仅作为纯底图」**。软件将 100% 完整保留原文档的所有背景色彩与文字，不会擦除任何高亮，也不会自动生成选框，直接作为普通背景信纸。
- **Word 模板带有高亮但想通篇全手写（懒得改模板）**：点击「导入 docx」选择文件后，若检测到高亮，在弹窗中选择**「全文手写模式」**。软件将自动忽略文档中的所有高亮标记，将全部文本统一作为纯手写体排版，无需手动清理 Word 模板格式。
...
- `roles` 字段（笔迹角色：各角色的字体、墨色、字号与扰动覆盖）为 Rust 版新增，
  随预设一并保存 / 载入；旧预设缺失时按空角色表载入，非法条目自动跳过
