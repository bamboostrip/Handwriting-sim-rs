...
- **彻底杜绝自动更新时弹出终端黑框窗口**：
  - 根因：Windows 11 的 Windows Terminal 会将直接由 `cmd.exe` 脱离启动的进程捕获并弹出独立可见终端窗口（`CREATE_NO_WINDOW` 与 `DETACHED_PROCESS` 在现代 conhost 下冲突导致窗体逃逸）。
  - 修复：更新引导脚本改由原生 Windows 脚本宿主（`wscript.exe //B //Nologo launcher.vbs`）静默拉起，通过 `WScript.Shell.Run ..., 0, False` 纯后台 0 窗口隐藏模式执行替换批处理，实现真正无感知静默更新。
- **修复更新失败场景下的临时下载与解压文件残留**：
  - 批处理在重试 20 次仍未能成功替换时的 `:copyfailed` 分支中，增加清理下载的新版可执行文件、压缩包及临时解压目录的操作，避免污染系统临时目录。
- **修复自动更新重启后软件以最小化状态启动的问题**：
  - 移除新版启动命令中的 `/min` 参数，更新完成后自动在前台正常展现新版窗口。
...
- **修复自动更新完成后循环弹出 ping 终端窗口**：
  - 根因：更新批处理用 `ping 127.0.0.1 -n 2` 做延时；父进程的 `CREATE_NO_WINDOW` 不会继承给孙进程，`ping.exe`（控制台程序）在无控制台情况下启动，Win11 默认终端会为每个 ping 弹一个新终端窗口，覆盖重试循环（最多 10 次）把它放大成“循环弹窗”。
  - 修复：延时改走 `wscript //B //Nologo` + `WScript.Sleep`（windows 子系统，永不分配控制台）；批处理其余命令均为 cmd 内部命令，不再产生任何控制台子进程。新增全平台回归测试锁定“bat 内容不得含 ping”。
  - 注意：从 0.4.0 更新到本版时会再经过一次旧版批处理（仍可能闪现 ping 窗口），此后彻底消失。
